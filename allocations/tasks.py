...
def deactivate_multiple_active_allocations_of_projects():
    for proj_id in (
        Allocation.objects.order_by().values_list("project_id", flat=True).distinct()
    ):
        project_active_allocations = list(
            Allocation.objects.filter(status="active", project_id=proj_id)
        )
