...
urlpatterns = [
    ...
    re_path(r'^(?P<pk>\d+)/delete/$', views.ApplianceDeleteView.as_view(), name='app_delete'),
    re_path(r'^create/$', views.app_create, name='app_create'),
    re_path(r'^create/image/$', views.app_create_image, name='app_create_image'),
    re_path(r'^template/(?P<resource>.+?)\.html/$', views.app_template, name='app_template'),
    re_path(r'^api/appliances/$', views.get_appliances, name='get_appliances'),
    re_path(r'^api/appliances/(?P<pk>\d+)/$', views.get_appliance, name='get_appliance'),
    re_path(r'^api/appliances/appliance_id/(?P<appliance_id>[0-9a-f-]+)/$', views.get_appliance_by_id, name='get_appliance_by_id'),
    re_path(r'^api/appliances/(?P<pk>\d+)/template$', views.get_appliance_template, name='get_appliance_template'),
    re_path(r'^api/keywords/$', views.get_keywords, name='get_keywords'),
    re_path(r'^api/keywords/(?P<appliance_id>\d+)/$', views.get_keywords, name='get_keywords'),
]
