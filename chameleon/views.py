...
from chameleon.keystone_auth import admin_ks_client, get_user, has_valid_token, WHITELISTED_PROJECTS
...
def migrate(request):
    ...
    if request.GET.get('force') or has_valid_token(request, region=token_region):
        ...
        return redirect(reverse('federation_confirm_legacy_credentials') + f'?{urlencode(params)}')
