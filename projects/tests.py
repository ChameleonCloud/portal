...
class ProjectViewTests(TestCase):
    ...
    def test_view_project_redirect(self):
        ...
        self.assertEqual(
            response["Location"], "http://testserver/login/?next=/user/projects/1234/"
        )
    ...
    def test_view_project(self, mock_tasclient_project, mock_tasclient_project_users):
        ...
        self.assertContains(
            response, 'data-allocation-id="1111" data-allocation-status="inactive"'
        )
        self.assertContains(
            response, 'data-allocation-id="1112" data-allocation-status="rejected"'
        )
        self.assertContains(
            response, 'data-allocation-id="1113" data-allocation-status="active"'
        )
    ...
    @mock.patch("pytas.http.TASClient.get_project_users")
    @mock.patch("pytas.http.TASClient.project")
    def test_view_project_with_pending(
        self, mock_tasclient_project, mock_tasclient_project_users
    ):
        projects_fixture = json.loads(
        ...
        self.assertContains(response, "Chameleon Project Request Test")
        self.assertContains(
            response, 'data-allocation-id="1114" data-allocation-status="active"'
        )
        self.assertContains(
            response, 'data-allocation-id="1115" data-allocation-status="pending"'
        )
    ...
    @mock.patch("pytas.http.TASClient.get_project_users")
    @mock.patch("pytas.http.TASClient.project")
    def test_view_allocation_rejected(
        self, mock_tasclient_project, mock_tasclient_project_users
    ):
        projects_fixture = json.loads(
        ...
        self.assertContains(response, "Chameleon Project Request Test")
        self.assertContains(
            response, 'data-allocation-id="1115" data-allocation-status="inactive"'
        )
        self.assertContains(
            response, 'data-allocation-id="1114" data-allocation-status="rejected"'
        )
    ...
    @mock.patch("pytas.http.TASClient.get_project_users")
    @mock.patch("pytas.http.TASClient.project")
    def test_view_allocation_rejected_with_active(
        self, mock_tasclient_project, mock_tasclient_project_users
    ):
        projects_fixture = json.loads(
            open("projects/test_fixtures/project_1234.json").read()
    ...
    @mock.patch("pytas.http.TASClient.fields")
    @mock.patch("pytas.http.TASClient.get_user")
    def test_allocation_form_fields_required(
        self, mock_tas_get_user, mock_tas_fields, mock_tas_project
    ):
        """
        Tests that appropriate fields are required on the project form.
        Args:
