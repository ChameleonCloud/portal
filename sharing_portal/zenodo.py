...
class ZenodoClient:
    ...
    def get_versions(self, doi):
        record = self._make_request(
            'records/{}'.format(ZenodoClient.to_record(doi)))
    ...
    def new_deposition_version(self, metadata=None, doi=None, file=None):
        if not (metadata and doi and file):
            raise ValueError('Missing required arguments, "metadata", "doi", and "file" are required')
        ...
        if not latest_url:
            raise ValueError('Could not discover latest version for deposition {}'.format(doi))
        ...
        if not draft_url:
            raise ValueError('Could not find created version draft for record {}'.format(latest_record))
...
class DepositionMetadata:
    def __init__(self, title=None, description=None, creators=None,
                 upload_type=None, publication_type=None, publication_date=None,
                 communities=None, keywords=None):
        self.title = title
