...
class RegisterViewTests(TestCase):
    ...
    def test_register_view(
        ...
        mock_get_department_choices.return_value = (
            (127, "Texas Advanced Computing Center"),
        )
    ...
    @mock.patch("tas.forms.get_department_choices")
    @mock.patch("tas.forms.get_institution_choices")
    def test_register_view_submit_not_pi(
        self,
        mock_get_institution_choices,
        mock_get_department_choices,
        mock_get_country_choices,
    ...
    @mock.patch("tas.forms.get_department_choices")
    @mock.patch("tas.forms.get_institution_choices")
    def test_register_view_submit_pi(
        self,
        mock_get_institution_choices,
        mock_get_department_choices,
        mock_get_country_choices,
...
class EditProfileViewTests(TestCase):
    def setUp(self):
        ...
        self.test_user = get_user_model().objects.create_user(
            fixture["username"], fixture["email"], "password"
        )
    ...
    def test_edit_profile_view(
        self,
        mock_terms_agreed_to_latest,
        mock_tas_authenticate,
        mock_tas_get_user,
        mock_get_institution_choices,
        mock_get_department_choices,
        mock_get_country_choices,
    ...
    def test_edit_profile_submit(
        ...
        response = self.client.post(
            reverse("tas:profile_edit"), mock_registration_form_data()
        )
...
class PasswordPolicy(TestCase):
    ...
    def test_password_character_class(self):
        valid, error = check_password_policy(
            self.test_user, "asdf;qwerty", "asdf;qwerty"
        )
        self.assertFalse(valid)
    ...
    def test_password_username_token(self):
        valid, error = check_password_policy(
            self.test_user, "asdf;JDOE1;qwer", "asdf;JDOE1;qwer"
        )
        self.assertFalse(valid)
    ...
    def test_password_full_name_token(self, mock_tas_get_user):
        mock_tas_get_user.return_value = test_user_fixture()
        valid, error = check_password_policy(
            self.test_user, "asdf;DOE1;qwer", "asdf;DOE1;qwer"
        )
    ...
    def test_password_valid(self, mock_tas_get_user):
        mock_tas_get_user.return_value = test_user_fixture()
        valid, error = check_password_policy(
            self.test_user, "a%4;D$$1!2qo(", "a%4;D$$1!2qo("
        )
...
class PasswordResetConfirmFormTests(TestCase):
    ...
    def test_password_reset_form_username(self, mock_tas_get_user):
        mock_tas_get_user.side_effect = Exception(
            "User not found", "User not found with username bad_username"
        )
