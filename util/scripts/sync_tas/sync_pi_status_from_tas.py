...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Simulate import, no changes applied",
    )
    parser.add_argument(
        "--users",
        type=str,
        help="Synchronize a specific user or set of users using a csv list of usernames",
        default=None,
    )
    parser.add_argument(
        "--seteligible",
        action="store_true",
        default=False,
        help="Used with --users flag to set all provided users as PI Eligible",
    )
...
def import_pi_status(dryrun=False, users=[], tas=None):
    ...
    for index, user in enumerate(users):
        ...
        logger.info(
            "{0} of {1}, {2}% complete, next: {3}".format(
        ...
        if user.pi_eligibility().lower() == "ineligible":
            ...
            if tas_pi_status.lower() != user.pi_eligibility().lower():
                ...
                logger.debug(
                    "{0} already synced: {1}".format(user.username, tas_pi_status)
                )
        else:
            logger.debug(
                "Skipping {0} portal pi eligibility is: {1}".format(
...
def set_all_pi_status(dryrun=False, users=[], status=None):
    ...
    for index, user in enumerate(users):
        ...
        if user.pi_eligibility().lower() == status.lower():
            logger.info(
                "pi_eligibility {0} already set for user {1}".format(
...
def get_tas_pi_status(tas, username):
    try:
        ...
        logger.error(
            "Error getting pi status for user: {0}".format(username), sys.exc_info()[0]
        )
        return "Ineligible"
...
def save_pi_status(user, status, dryrun=False):
    ...
    if dryrun:
        logger.info(
            "Dry run, TAS PI Status {0} for user {1} not updated".format(
                status, user.username
            )
        )
        return
