...
def get_projects_from_tas(tas, db):
    ...
    for proj in resp:
            ...
            if key in TAS_TO_PORTAL_MAP and key not in IGNORED_TAS_FIELDS:
                if key == "type":
                    ...
                    if val not in type_name_to_id:
                        cursor.execute(
                            "INSERT INTO {table} (name) VALUES ('{val}')".format(
                                table=PORTAL_TYPE_TABLE_NAME, val=val
                            )
                        )
                        db.commit()
                ...
                if key == "field":
                    ...
                    if val and val not in field_name_to_id:
                        cursor.execute(
                            "INSERT INTO {table} (name) VALUES ('{val}')".format(
                                table=PORTAL_FIELD_TABLE_NAME, val=val
                            )
                        )
                        db.commit()
                ...
                if key == "piId":
                    ...
                    cursor.execute(
                        "SELECT id FROM {table} WHERE username = '{username}'".format(
                            table=PORTAL_AUTH_USER_TABLE_NAME, username=username
                        )
                    )
        ...
        if tas_proj_nickname:
            ...
            cursor.execute(
                "SELECT nickname FROM {table} WHERE tas_project_id = '{tas_project_id}'".format(
                    table=PORTAL_PROJECT_EXTRA_TABLE_NAME, tas_project_id=tas_proj_id
                )
            )
            ...
            if nickname:
                ...
                logger.warning(
                    "No nickname found in portal for project with tas id {}. The charge code will be used.".format(
                        tas_proj_id
                    )
                )
...
def sync(db, tas_projects, portal_projects):
    ...
    for proj_charge_code in list(tas_projects.keys()):
        if not columns:
            columns = list(tas_projects[proj_charge_code].keys())
        if proj_charge_code in portal_projects:
            ...
            if tas_projects[proj_charge_code] != portal_projects[proj_charge_code]:
                proj_values = [
                    tas_projects[proj_charge_code][key]
                    for key in sorted(tas_projects[proj_charge_code].keys())
                ]
    ...
    if columns:
        insert_query = """INSERT INTO {table} ({columns}) 
            ...
            table=PORTAL_PROJECT_TABLE_NAME,
    ...
    if columns:
        ...
        update_query = """UPDATE {table} SET {variables} WHERE id = %s""".format(
            table=PORTAL_PROJECT_TABLE_NAME, variables=",".join(values)
        )
...
def sync_fields_from_tas(tas, db):
    ...
    cursor.execute(
        """INSERT IGNORE INTO {table} (name) VALUES {values}""".format(
            table=PORTAL_FIELD_TABLE_NAME, values=",".join(field_names)
        )
    )
    db.commit()
    ...
    cursor.execute(
        """INSERT IGNORE INTO {table} (parent_id, child_id) VALUES {values}""".format(
            table=PORTAL_FIELD_HIERARCHY_TABLE_NAME,
