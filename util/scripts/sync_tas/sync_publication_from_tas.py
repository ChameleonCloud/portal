        ...
        if tas_proj_id and charge_code:
            cursor.execute(
                "SELECT id FROM {table} WHERE charge_code = '{charge_code}'".format(
                    table=PORTAL_PROJECT_TABLE_NAME, charge_code=charge_code
                )
            )
            ...
            if portal_proj_id:
                ...
                logger.warning(
                    "Couldn't find project with charge code {} in portal".format(
                        charge_code
                    )
                )
...
def sync(db, tas_to_portal_project_id_map):
    cursor = db.cursor(dictionary=True)
    update_query = """UPDATE {table} SET project_id = %s WHERE id = %s""".format(
        table=PORTAL_PROJECT_PUBLICATION_TABLE_NAME
    )
    cursor.execute(
        "SELECT id, tas_project_id FROM {table}".format(
            table=PORTAL_PROJECT_PUBLICATION_TABLE_NAME
        )
    )
        ...
        if tas_proj_id:
            if tas_proj_id in tas_to_portal_project_id_map:
                try:
                    ...
                    logger.exception(
                        "failed to update record {} in publication table".format(
                            row["id"]
                        )
                    )
            else:
                logger.warning(
                    "Counld't find mapping from tas project {} to portal project".format(
                        tas_proj_id
                    )
                )
