...
def download_registrants(ModelAdmin, request, queryset):
    ...
    try:
        for obj in queryset:
            participants = models.WebinarRegistrant.objects.filter(webinar__id=obj.id)
            content = list(p.user.first_name + ", " + p.user.last_name + ", " + p.user.username + ", " + p.user.email for p in participants)
        response = HttpResponse('\n'.join(content), content_type='text/plain')
        response['Content-Disposition'] = 'attachment; filename="webinar_%s_participants.txt"' % id
    except Exception as e:
        response = HttpResponse('Error: %s' % e)
...
class WebinarAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'start_date', 'end_date', 'registration_open', 'registration_closed', 'number_of_registrants', 'registration_limit', 'registrants')
