...
def index(request):
    ...
    if len(webinars) == 1:
        return HttpResponseRedirect(
            reverse("webinar_registration:webinar", args=(webinars[0].id,))
        )
...
def webinar(request, id):
    ...
    try:
        participant = models.WebinarRegistrant.objects.get(
    ...
    return render(
        request,
        "webinar_registration/webinar.html",
        {"webinar": webinar, "participant": participant, "is_full": is_full},
    )
...
def register(request, id):
    ...
    if not webinar.is_registration_open():
        return HttpResponseRedirect(
            reverse("webinar_registration:webinar", args=(webinar.id,))
        )
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if num_registrants == webinar.registration_limit:
                send_mail(
                    "Registration limit reached for webinar " + webinar.name,
                    "The registration limit for your webinar, "
                    + webinar.name
                    + ", has been reached.",
        ...
        context["form"] = forms.WebinarRegistrantForm(
            initial={"user": request.user, "webinar": webinar}
        )
...
def unregister(request, id):
    ...
    if request.POST:
        form = forms.WebinarRegistrantForm(request.POST)
        if form.is_valid():
            cancel_object = models.WebinarRegistrant.objects.get(
                user=form.cleaned_data["user"], webinar=form.cleaned_data["webinar"]
            )
